...
def get_experiment(args, device):
    ...
    if args.experiment == 'MNIST_MLP':
        ...
        model = MLP(np.prod(input_dim), eta=args.eta, variance=args.variance).to(device)
        ...
        model = ConvNet(in_channels=input_dim[0], eta=args.eta, variance=args.variance).to(device)
