...
class Decorrelation(nn.Module):
    ...
    def __init__(self, in_features: int, decor_lr: float = 0.0, kappa: float = 0.5, 
                 downsample_perc: float = 1.0, latent_factor=0.5, device=None, dtype=None) -> None:
        ...
        self.reduced_length = int(in_features * latent_factor)

        self.u = torch.nn.Parameter(torch.empty((in_features, self.reduced_length)), requires_grad=False)
        
        self.register_buffer("weight", torch.empty(self.in_features, self.in_features, **factory_kwargs))

        self.reset_parameters()
        self.recompose_r = True
    
    def reset_parameters(self):

        def _generate_orthonormal_matrix():
            std = 0.1
            random_matrix = torch.randn(self.in_features, self.reduced_length) * std

            Q, _ = torch.linalg.qr(random_matrix)

            # reduced_Q = Q[:, :int(self.in_features * self.latent_factor)]

            return Q

        self.u.data = _generate_orthonormal_matrix()
        
        self.weight.data = self.u @ self.u.T
    ...
    def update(self, loss_only=False):
        ...
        v = torch.mean(torch.diag(X) - 1.0, axis=0)

        if not loss_only:
            self.u.data -= self.decor_lr * ((1.0 - self.kappa) * C @ self.u + 2 * self.kappa * v * self.u)
            # self.u.grad = update / len(self.u)
...
class DecorConv2d(Decorrelation):
    ...
    def __init__(self, in_channels: int, out_channels: int, kernel_size: _size_2_t,
                 ...
                 bias: bool = True, decor_lr: float = 0.0, kappa=1e-3, method=None, full=True,
                 downsample_perc=1.0, latent_factor = 0.5, device=None, dtype=None) -> None:
        ...
        Args:
            ...
            dtype (optional): Data type of the input.
        """
        super().__init__(in_features=in_channels * np.prod(kernel_size), decor_lr=decor_lr, kappa=kappa, downsample_perc=downsample_perc, latent_factor=latent_factor, device=device, dtype=dtype)        
...
class DecorConvTranspose2d(Decorrelation):
    ...
    def __init__(self, in_channels: int, out_channels: int, kernel_size: _size_2_t,
                 ...
                 downsample_perc=1.0, latent_factor=0.5, device=None, dtype=None, weights=None, loaded_bias=None) -> None:
